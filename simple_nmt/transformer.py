...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        z = self.norm(x + self.attn(x, x, x, mask=mask))
        z = self.fc_norm(z + self.fc(z))
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, KV, mask, prev):
        if prev is not None: # prev should be list.
            ...
            forward_mask = torch.triu(torch.ones(x.size(1), x.size(1)), diagonal=1).byte().to(x.device)
            # |forward_mask| = (m, m)
            forward_mask = forward_mask.unsqueeze(0).expand(x.size(0), *forward_mask.size())
            # |forward_mask| = (batch_size, m, m)

            z = self.masked_attn_norm(x + self.masked_attn(x, x, x, mask=forward_mask))
            # |z| = (batch_size, m, hidden_size)

        # |KV| = (batch_size, n, hidden_size)
        # |mask| = (batch_size, m, n)
        z = self.attn_norm(z + self.attn(Q=z, K=KV, V=KV, mask=mask))
        # |z| = (batch_size, m, hidden_size)
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = MySequential(
            *[EncoderBlock(hidden_size, n_splits) for _ in range(n_enc_blocks)]
        )
        self.decoder = MySequential(
            *[DecoderBlock(hidden_size, n_splits) for _ in range(n_dec_blocks)],
        )
