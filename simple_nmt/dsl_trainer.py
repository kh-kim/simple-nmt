...
class DualSupervisedTrainer():
    ...
    @staticmethod
    def _reordering(x, y, y_length):
        indice = y_length.topk(y_length.size(0))[1]

        x_ = x.index_select(dim=0, index=indice).contiguous()
        y_ = y.index_select(dim=0, index=indice).contiguous()
        y_length_ = y_length.index_select(dim=0, index=indice).contiguous()

        restore_indice = (-indice).topk(y_length.size(0))[1]

        return x_, (y_, y_length_), restore_indice
    ...
    def _get_loss(x, y, x_hat, y_hat, crits, x_lm=None, y_lm=None, lagrange=1e-3):
        ...
        return loss_x2y.sum(), loss_y2x.sum(), dual_loss.sum() if dual_loss is not None else .0
